  ...
  parse: function (text, baseUrl) {
    ...
    function parseGeometry(xml) {
      ...
      for (var i = 0; i < mesh.childNodes.length; i++) {
        ...
        switch (child.nodeName) {
          case "source":
            data.sources[child.getAttribute("id")] = parseFloats(
              child.getElementsByTagName("float_array")[0].textContent
            );
            break;

          case "vertices":
            data.sources[child.getAttribute("id")] =
              data.sources[
                parseId(
                  child.getElementsByTagName("input")[0].getAttribute("source")
                )
              ];
            break;
          ...
          case "polygons":
            console.log(
              "ColladaLoader: Unsupported primitive type: ",
              child.nodeName
            );
            break;
          ...
          switch (name) {
            case "VERTEX":
              geometry.addAttribute(
                "position",
                new THREE.Float32Attribute(array, 3)
              );
              break;
    ...
    var version = xml
      .getElementsByTagName("COLLADA")[0]
